...
export class Tab {
  ...
  public delete(team: Team): boolean {
    if (!this.teams.has(team))
      throw new Error("This team doesn't belong to this tab");
    else {
  ...
  public metrics(): Metrics {
    ...
    return {
      average: Math.avg(
        Array.from(
          this.teams.map<number>(
            (team) => team.metrics().combined
  ...
  public healthCheck(players: Set<Player>): boolean {
    ...
    if (playersInTeam.size !== players.size) {
      ...
      console.error(
        "PARTICIPANTS : ",
        JSON.stringify(Array.from(players), null, 2),
        "\n"
      );
      throw new Error(
        `Team generation failed to add all players in teams, ${players.size} participants vs ${playersInTeam.size} players in teams`
      );
    }
